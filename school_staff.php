...
<tbody>
    <?php
     ...
     $sql="SELECT COUNT(*) as total_students from school_register where user_category='staff'";
     $result=$connection->query($sql);
     if($result->num_rows>0){
      $row=$result->fetch_assoc();
      echo"<h1>Total students:  " .$row["total_students"]."</h1>";
      }
     $sql="SELECT * From school_register where user_category='staff'";
     $result= $connection->query($sql);
     if(!$result){
    ...
    <td>$row[fee]</td>
    <td><a class='btn btn-primary btn-sm' href='/Transport-project/school-edit-fee2.php?roll_number=$row[roll_number]'>Edit</a>
