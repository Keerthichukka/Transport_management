...
if ($_SERVER['REQUEST_METHOD']=='GET')
{
    ...
    $S_NO=$_GET["S_NO"];
    $sql="SELECT * FROM school WHERE S_NO=$S_NO";
    ...
    $BUS_NUMBER = $_POST['BUS_NUMBER'];
       ...
       $STARTING_POINT=$_POST['STARTING_POINT'];
        ...
        do{
            if(empty($MILEAGE)||empty($INSURANCE_DUE_DATE)||empty($PERMIT_DUE_DATE)||empty($STARTING_POINT)||empty($BUS_NUMBER)|| empty($BUS_DRIVER)|| empty($CONTACT_NUMBER)||empty($STOPS))
            {
               $errorMessage="All the fileds are required";
               break;
            }
            $S_NO=$_GET["S_NO"];
            $sql=("UPDATE school SET BUS_NUMBER='$BUS_NUMBER',BUS_DRIVER='$BUS_DRIVER',CONTACT_NUMBER='$CONTACT_NUMBER',STOPS='$STOPS',NO_OF_PASSENGERS='$NO_OF_PASSENGERS',MILEAGE='$MILEAGE',PERMIT_DUE_DATE='$PERMIT_DUE_DATE',INSURANCE_DUE_DATE='$INSURANCE_DUE_DATE',STARTING_POINT='$STARTING_POINT' WHERE S_NO=$S_NO")or
    die($mysqli->error);   
            $result=$connection->query($sql);
            if (!$result)
            {
