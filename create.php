...
{       $S_NO=$_POST['S_NO'];
       ...
       $STARTING_POINT=$_POST['STARTING_POINT'];
        ...
        do{
         if(empty($MILEAGE)||empty($INSURANCE_DUE_DATE)||empty($PERMIT_DUE_DATE)||empty($STARTING_POINT)||empty($FEE)||empty($S_NO)||empty($BUS_NUMBER)||empty($BOARDING_TIME)|| empty($BUS_DRIVER)|| empty($CONTACT_NUMBER)||empty($STOPS))
         {
            $errorMessage="All the fileds are required";
            break;
         }
         $sql = "INSERT INTO view (S_NO,BUS_NUMBER,BOARDING_TIME,BUS_DRIVER,CONTACT_NUMBER,STOPS,NO_OF_PASSENGERS,MILEAGE,PERMIT_DUE_DATE,INSURANCE_DUE_DATE,STARTING_POINT,FEE) VALUES ('$S_NO','$BUS_NUMBER','$BOARDING_TIME','$BUS_DRIVER','$CONTACT_NUMBER','$STOPS','$NO_OF_PASSENGERS','$MILEAGE','$PERMIT_DUE_DATE','$INSURANCE_DUE_DATE','$STARTING_POINT','$FEE')";
        $result=$connection->query($sql);
        if(!$result)
        {
...
<div class="col-sm-3 d-grid">
    <a class="btn btn-outline-primary" href="/Transport-project/viewcontent.php" role="button">Cancel</a>
</div>
</div>
</form>
</div>
