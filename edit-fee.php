...
if ($_SERVER['REQUEST_METHOD']=='GET')
{
    if (!isset($_GET["roll_number"]))
    {
       ...
       header("location:/transport-project/staff.php");
        exit;
    }
    $roll_number=$_GET["roll_number"];
   
    $sql="SELECT * FROM staff WHERE roll_number='$roll_number'";   
...
{header("location:/transport-project/staff.php");
    ...
    # code...
        ...
        do{
            ...
            $roll_number=$_GET["roll_number"];
            $sql=("UPDATE staff SET fee='$fee' WHERE roll_number='$roll_number'")or
    die($mysqli->error);   
            $result=$connection->query($sql);
            if (!$result)
            {
            ...
            $sql = "SELECT BUS_NUMBER, TOTAL_INCOME ,PROFIT FROM view";
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $sql2 = "SELECT fee AS total_fee FROM staff WHERE bus_number = '$bus_id'";
                ...
                $bus_id = $row["BUS_NUMBER"];
                $sql3 = "UPDATE view SET TOTAL_INCOME = '$income' ,PROFIT='$profit' WHERE BUS_NUMBER = '$bus_id'";
...
<div class="col-sm-3 d-grid">
    <a class="btn btn-outline-primary" href="/transport-project/staff.php" role="button">Cancel</a>
</div>
</div>
</form>
</div>
