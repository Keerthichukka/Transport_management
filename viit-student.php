...
<tbody>
    <?php
     ...
     $sql="SELECT COUNT(*) as total_students from student where college_name='viit'";
     $result=$connection->query($sql);
     if($result->num_rows>0){
      $row=$result->fetch_assoc();
      echo"<h1>Total students:  " .$row["total_students"]."</h1>";
      }
     $sql="SELECT * From student where college_name='viit'";
     $result= $connection->query($sql);
     if(!$result){
    ...
    <td>$row[fee]</td>
    <td><a class='btn btn-primary btn-sm' href='/Transport-project/edit-fee2.php?roll_number=$row[roll_number]'>Edit</a>
    <a class='btn btn-danger btn-sm' href='/Transport-project/student-delete.php?roll_number=$row[roll_number]'>Delete</a></td>
