            ...
            if ($_SERVER['REQUEST_METHOD']=='GET')
{
    ...
    $BUS_NUMBER=$_GET["bus_number"];
     $sql="SELECT COUNT(*) as total_students from student where  fee='0' and BUS_NUMBER='$BUS_NUMBER'";
     $result=$connection->query($sql);
     $sql2="SELECT COUNT(*) as total_students from student where  fee='0' and BUS_NUMBER='$BUS_NUMBER'";
     $result1=$connection->query($sql2);
     if($result->num_rows>0 and $result1->num_rows>0){
      $row=$result->fetch_assoc();
      $row1=$result1->fetch_assoc();
      echo"<h1>Total students:  " .$row["total_students"]+$row1["total_students"]."</h1>";
      }
     ...
     if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
          if ($row["fee"] == "" or $row["fee"] == "0") { //assuming the total amount is 1000
    ...
    <td>$row[fee]</td>
    <td><a class='btn btn-primary btn-sm' href='/Transport-project/edit-fee2.php?roll_number=$row[roll_number]'>Edit</a></td>
    </tr>
        ";
     }
    }
