            ...
            if ($_SERVER['REQUEST_METHOD']=='GET')
{
 ...
 echo" &nbsp&nbsp&nbsp
  <a href='http://localhost/Transport-project/paid.php?bus_number=$BUS_NUMBER ' ><button id='bottone1' style='height:50px; padding:5px; font-size:12px;'>PAID</button></a>
  &nbsp&nbsp&nbsp
  &nbsp&nbsp&nbsp
  <a href='http://localhost/Transport-project/notpaid.php?bus_number=$BUS_NUMBER'><button id='bottone1' style='height:50px; padding:5px; font-size:12px;'>DUES</button></a>
  &nbsp&nbsp&nbsp";
}?>
  <BR>
            ...
            if ($_SERVER['REQUEST_METHOD']=='GET')
{
    ...
    $BUS_NUMBER=$_GET["BUS_NUMBER"];
    $sql="SELECT COUNT(*) as total_students from student where bus_number='$BUS_NUMBER'";
      ...
      <table class="table" style="width:50%;font-size: 13px;  text-align: center; float:left;">
        <threa >
          <tr>
            ...
            <th  style=" font-weight: bold;">CURRENT YEAR</th>
    ...
    if (!isset($_GET["BUS_NUMBER"]))
    {
       ...
       header("location:/transport-project/income.php");
        exit;
            ...
            $sql="SELECT * FROM `student` where bus_number='$BUS_NUMBER' ";
            $result= $connection->query($sql);
            if(!$result){
            ...
            if ($_SERVER['REQUEST_METHOD']=='GET')
{
    ...
    $BUS_NUMBER=$_GET["BUS_NUMBER"];
            ...
            $sql="SELECT * FROM `staff` where bus_number='$BUS_NUMBER' ";
            $result= $connection->query($sql);
            if(!$result){
              die("Invalid query: ".$connection->error);
            }
